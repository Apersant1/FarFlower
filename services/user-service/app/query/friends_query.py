...
async def get_request_list(userID: uuid.UUID,
    ...
    req_list = await db.execute(select(FriendRequest)
...
async def accept_request(requestID: uuid.UUID,
    ...
    if request is None:
        return 1
    if request.status == FriendRequestStatus.pending and status == FriendRequestStatus.accepted:
        request.status = status
        await db.commit()
        return request
    if request.status == FriendRequestStatus.accepted and status == FriendRequestStatus.accepted:
        return 2
    if request.status == 'accepted' and status == FriendRequestStatus.rejected:
        await db.execute(delete(FriendRequest).where(FriendRequest.id == requestID))
        await db.commit()
        return 3
