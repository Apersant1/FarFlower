...
from app.schemas import FriendRequestSchemaCreate, FriendRequestSchemaRead, FriendRequestStatus,UserRead
from app.query import send_request, get_request_list, accept_request, get_friends_list
...
async def send_friend_request(friendID: uuid.UUID,
                              db: AsyncSession = Depends(get_async_session),
                              current_user: dict = Depends(current_active_user)
):
    answer = await send_request(friendID=friendID, db=db, current_user=current_user)
    if answer == 0:
...
async def get_requests_list(db: AsyncSession = Depends(get_async_session),
                            current_user: dict = Depends(current_active_user)
):
...
async def accepting_request(requestID: uuid.UUID,
    ...
    request = await accept_request(requestID=requestID, status=status, db=db, current_user=current_user)
    if request == 0:
        return HTTPException(status_code=403, detail="Forbidden: invalid user")
    elif request == 1:
        ...
        return HTTPException(status_code=200, detail="Request was rejected")
    return request

@FriendRouter.get("/friend/friend-list", tags=["Friend"],
                  response_model=List[UserRead])
async def get_friend_list(current_user: dict = Depends(current_active_user),
                          db: AsyncSession = Depends(get_async_session)):
